...
def simple_enemy(enemy, player, cooldown, obstacles, enemies):
...
def hunter(enemy, player, cooldown, obstacles, enemies):
...
def sniper(enemy, player, cooldown, obstacles, enemies):
...
def guard(enemy, player, cooldown, obstacles, enemies):
    d_delta, dx, dz = get_distance(enemy, player)
    ...
    if d_delta < PATROL_RADIUS * 4:
        ...
        enemy.rotate_towards(dx, dz)
        if d_delta > 5.0:
            pathfind_step(enemy, player, obstacles, enemies)
        return shot_attempt(
            enemy,
            d_delta,
    ...
    if home_distance > PATROL_RADIUS and enemy.state != "patrol":
        ...
        class HomeTarget:
            x = enemy.home_x
            z = enemy.home_z
        pathfind_step(enemy, HomeTarget(), obstacles, enemies)
    else:
        # State: patrol
        enemy.state = "patrol"
...
#             enemy, 
#             d_delta, 
#             range=STANDARD_RANGE, # regular range
#             cooldown=cooldown # regular cooldown
#         )
